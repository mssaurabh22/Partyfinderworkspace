...
public class UpcomingEventParser extends DefaultHandler {

	private ArrayList<EventItem> upcomingEvent;
	...
	public UpcomingEventParser() {
		upcomingEvent = new ArrayList<EventItem>();
	}

	public ArrayList<EventItem> getUpcomingEvents(){
	...
	public void endElement(String uri, String localName, String qName)
		...
		/*			if(qName.equalsIgnoreCase(EVENT_DETAIL)){
			//add it to the ArrayList
				upcomingEvent.add(mUpcomingEventItem);

		}else if(qName.equalsIgnoreCase(IMAGE_URL)){
			mUpcomingEventItem.setVcImageUrl(tempVal);
			Log.d("", "ImageUrl :"+ tempVal);
		}else if(qName.equalsIgnoreCase(COMPANY_CODE)){
			mUpcomingEventItem.setVcCompanyCode(tempVal);
			Log.d("", "ImageUrl :"+ tempVal);
		}else if(qName.equalsIgnoreCase(EVENT_NAME)){
			mUpcomingEventItem.setVcEventName(tempVal);
			Log.d("", "ImageUrl :"+ tempVal);


		}else if(qName.equalsIgnoreCase(EVENT_ADDRESS)){
			mUpcomingEventItem.setVcEventAdd(tempVal);
			Log.d("", "ImageUrl :"+ tempVal);


		}else if(qName.equalsIgnoreCase(EVENT_DATE)){
			mUpcomingEventItem.setDtEventDate(tempVal);
			Log.d("", "ImageUrl :"+ tempVal);
		}else if(qName.equalsIgnoreCase(EVENT_START_TIME)){
			mUpcomingEventItem.setDtEventStartTime(tempVal);
			Log.d("", "ImageUrl :"+ tempVal);
		}else if(qName.equalsIgnoreCase(EVENT_END_TIME)){
			mUpcomingEventItem.setDtEventEndTime(tempVal);
			Log.d("", "ImageUrl :"+ tempVal);
		}else if(qName.equalsIgnoreCase(EVENT_CODE)){
			mUpcomingEventItem.setVcEventCode(tempVal);
		}else if(qName.equalsIgnoreCase(COMPNY_NAME)){
			mUpcomingEventItem.setVcCompanyName(tempVal);
		}else if(qName.equalsIgnoreCase(EVENT_CITY)){
			mUpcomingEventItem.setVcEventCity(tempVal);
		}else if(qName.equalsIgnoreCase(EVENT_STATE)){
			mUpcomingEventItem.setVcEventState(tempVal);
		}*/

		tempVal=tagVal.toString();
		if(qName.equalsIgnoreCase(EVENT_DETAIL)){
			//add it to the ArrayList
			upcomingEvent.add(mEvenItem);
