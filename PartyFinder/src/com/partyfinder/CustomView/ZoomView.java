...
public class ZoomView extends FrameLayout{
	...
	private String TAG ="ZoomViewClass";
	...
	private void processSingleTouchOutsideMinimap(final MotionEvent ev) {
		...
		/*	if (l < 30.0f) {
                ...
                if (System.currentTimeMillis() - lastTapTime < 500) {
                    if (smoothZoom == 1.0f) {
                    	Log.i("",TAG+"1st Condition is running...");
                        smoothZoomTo(maxZoom, x, y);
                    	Log.i("",TAG+"mazZoom Value  : "+maxZoom);
                    } else {
                        smoothZoomTo(1.0f, getWidth() / 3.0f, getHeight() / 3.0f);
                    	Log.i("",TAG+"second Condition is running..."+maxZoom+"height :"+getHeight() + "Width :" +getWidth());
//                    }
                    lastTapTime = 0;
                    ev.setAction(MotionEvent.ACTION_CANCEL);
                    super.dispatchTouchEvent(ev);
	...
	private void processDoubleTouchEvent(final MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			if (pinching || ld > 30.0f) {
				pinching = true;
				final float dxk = 0.15f * (dx1 + dx2);
				final float dyk = 0.15f * (dy1 + dy2);
