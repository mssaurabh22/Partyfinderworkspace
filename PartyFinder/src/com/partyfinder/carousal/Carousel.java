...
import com.partyfinder.customInterface.WhatsHot;
...
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener {
	 ...
	 * Max theta
	 */
	private static final float MAX_THETA = 8.0f;
	 ...
	 * The axe angle
	 */
    private float mTheta = (float)(-10f*(Math.PI/180.0));	 
   ...
   private ArrayList< EventItem> mWhatHot;
   private WhatsHot mWhathotListener;
	public int mScreenWidth;
	public int mScreenHeight;
	public WhatsHot getmWhathotListener() {
	return mWhathotListener;
}

public void setmWhathotListener(WhatsHot mWhathotListener) {
	this.mWhathotListener = mWhathotListener;
}
	...
	public Carousel(Context context)  {
		...
		Log.i("", "screen resolution  Carousel : " +"width : "+mScreenWidth +"height : "+mScreenHeight);
		Log.i("","Single constructor is calling....");
	}
	
	public Carousel(Context context, AttributeSet attrs)  {
		this(context, attrs, 0);
		Log.i("","Double constructor is calling....");
	...
	public Carousel(Context context, AttributeSet attrs, int defStyle) {
		...
		mWhatHot=WhatsHotFragment.mWhathotList;
		Log.i("", "Triple constructor is calling : "+mWhatHot);
		...
		// Retrieve settings
//		TypedArray arr = getContext().obtainStyledAttributes(R.styleable.Carousel);
		mAnimationDuration =300;// arr.getInteger(R.styleable.Carousel_android_animationDuration, 400);
		mUseReflection = false;//arr.getBoolean(R.styleable.Carousel_UseReflection, true);	
		int selectedItem = 0;//arr.getInteger(R.styleable.Carousel_SelectedItem, 0);
		Log.i("","Image SelectedItem :  "+selectedItem);
		int imageArrayID =-1 ;//arr.getResourceId(R.styleable.Carousel_Items, -1);	
		Log.i("","Image ArrayId :  "+imageArrayID);
...
//		if(mWhatHot)
		int min =MIN_QUANTITY; //arr.getInteger(R.styleable.Carousel_minQuantity, MIN_QUANTITY);
		int max =MAX_QUANTITY; //arr.getInteger(R.styleable.Carousel_maxQuantity, MAX_QUANTITY);
		
		float mTheta =MAX_THETA;// arr.getFloat(R.styleable.Carousel_maxTheta, MAX_THETA);
...
/*		if(mWhatHot.size() < mMinQuantity || mWhatHot.size() > mMaxQuantity)
			throw new IllegalArgumentException("Invalid set of items.");*/		
		// Initialize image adapter
		ImageAdapter adapter = new ImageAdapter(getContext());
		
//		adapter.SetImages(images, names, mUseReflection);
		adapter.setImagegList(mWhatHot,mUseReflection);
	    setAdapter(adapter);
...
//childCount = childCount+6;
	...
	private class ImageAdapter extends BaseAdapter {
		...
		private ArrayList<EventItem> imageList;
		
		public ImageAdapter(Context c) {
			mContext = c;
		}		
		
		public void setImagegList(ArrayList<EventItem> mImageList,boolean reflected){
			imageList=mImageList;
			Log.i("","Image List Size : "+imageList.size());
			mImages = new CarouselItem[imageList.size()];
			for(int i = 0; i< imageList.size(); i++)
			{
//				drawables[i] = array.getDrawable(i);
//				Bitmap originalImage = imageList.get(i).getImageName();//((BitmapDrawable)drawables[i]).getBitmap();
		...
		/*		if(reflected){
				...
				final CarouselItem item = new CarouselItem(mContext,imageList.get(i).getVcImageUrl());
				item.setIndex(i);
...
//						Log.i("","Clicked Item : "+item.getIndex());
//					}
//				});
				/*if(names != null)
					item.setText(names.getString(i));*/
//				mWhathotListener.scrollWhatshot(imageList.get(i));
				mImages[i] = item;
...
//			View itemTemplate = inflater.inflate(R.layout.item, null);//inflate(R.layout.item, this, true);
//			final CarouselItem item = new CarouselItem(mContext,imageList.get(position).getVcImageUrl());
//			item.setIndex(i);
...
/*
    	float x = - (float)(diameter/0.9f  * android.util.FloatMath.sin(angleOffset)) + diameter/2 - child.getWidth()/2;
    	float z = diameter/0.9f * (1.0f - (float)android.util.FloatMath.cos(angleOffset));
    	float y = - getHeight()/3f + (float) (z * android.util.FloatMath.sin(mTheta));*/

    	
    	float x = - (float)(diameter/3  * android.util.FloatMath.sin(angleOffset)) + diameter/2 - child.getWidth()/2;
    	float z = diameter/3 * (1.0f - (float)android.util.FloatMath.cos(angleOffset));
    	float y = - getHeight()/3f + (float) (z * android.util.FloatMath.sin(mTheta));
    ...
    private int calculateTop(View child, boolean duringLayout) {
        ...
        case Gravity.CENTER_VERTICAL:
            ...
            childTop = mSpinnerPadding.top + (availableSpace / 5);
            break;
        case Gravity.BOTTOM:
...
//       int CarouselHeight = getResources().getDimensionPixelSize(R.dimen.whatsHot_carousel_hieght);
//       int CarouselWidth = getResources().getDimensionPixelSize(R.dimen.whatsHot_carousel_width);
       
       
    // line no. 1477  Carousel 
   	
       int CarouselWidth = 0;
    int CarouselHeight = 0;
    
    if(mScreenHeight>920 && mScreenHeight<980){
    	if(mScreenWidth>520 && mScreenWidth<560){
   CarouselHeight = 530;
   CarouselWidth = 350;
    	}
    }else{
    	  CarouselHeight = getResources().getDimensionPixelSize(R.dimen.whatsHot_carousel_hieght);
          CarouselWidth = getResources().getDimensionPixelSize(R.dimen.whatsHot_carousel_width);
    }
        ...
        child.layout(childLeft, childTop, CarouselWidth, CarouselHeight);
        //250,380hdpi//320,500xhdpi//
